...
2. **Content Retrieval**: Fetches each URL's text via Tavily Extract (with `markdown.new` as fallback); each block is then compressed query-aware (conditioned on the pipeline `topic`) via `utils/content/compressor.py` → list of compressed text blocks
